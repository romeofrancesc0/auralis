...
class AttentionModule:
    ...
    def select_stream(
        ...
        attention module decides which stream to attend to.

        Args:
            streams:       two separated waveforms (arbitrary permutation)
            target_gender: 0=Female, 1=Male
            sr:            sample rate
            method:        "classifier" — MLP+GMM score (default, trained on LibriSpeech);
                           "pitch"      — mean F0 comparison (language-agnostic, no model).

        Returns:
            (selected_stream, confidence) — confidence is the absolute difference
            between the two stream scores; near 0 = ambiguous selection.
        """
        if method == "pitch":
            pitch_a = _mean_active_pitch(streams[0], sr)
            pitch_b = _mean_active_pitch(streams[1], sr)
            # Female → higher F0; Male → lower F0
