"""Extended evaluation — male target (MLP + GMM + HMM + DPCRN) on 36 samples.

36 samples = 12 per SNR level (-3, 0, +3 dB), drawn with seed=123.
Reports mean +- std for SI-SDR, PESQ, and STOI.
Reference signal is the male speaker (sample.interferer).
"""
...
def run_pipeline(mix: np.ndarray, attention: AttentionModule, dpcrn: DPCRN) -> np.ndarray:
    mask = attention.compute_mask(mix, sr=SR, target_gender=1)
    recon = separate_nmf(mix, mask, sr=SR, mask_net=dpcrn, target_gender=1)
...
def main() -> None:
    ...
    for snr in SNR_LEVELS:
        ...
        for i, sample in enumerate(samples):
            mix    = sample.mixture
            target = sample.interferer  # male voice is the target here
    ...
    print()
    print("=" * W)
    print("  AURALIS — EXTENDED EVALUATION (MLP + GMM + HMM + DPCRN) — MALE TARGET")
